...
#convenience method for testing to see if a file is a video by path name
def file_is_video(fileName):
    f = open(fileName, "rb")
    bytes = f.read(2)
    return is_video(bytes)
...
class Snapchat(object):
    ...
    def append_story(self, media_id, isVideo, time=5):
        '''Appends media to your snapchat story.
	    Returns true on success

	    :param media_id: The id of the media to append to the story
	    :param client_id: The id of the client
	    :param isVideo: (1 -> Video, 0 -> Image). True if video.
	    :param time: The amount of time to set the story to.
	    '''
        r = self._request('post_story',{
            ...
            "caption_text_display" : "Sent by groupsnap."})
        if len(r.content) > 0:
            print("Got response: " + r.content)
            return r.json()
        else:
            return False
